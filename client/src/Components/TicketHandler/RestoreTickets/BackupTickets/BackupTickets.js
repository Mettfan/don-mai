...
function BackupTickets(props) {
    ...
    let [tickets, setTickets] = useState(props.tickets || allTickets || testTickets)
    useEffect(() => {
    ...
    function sortTickets(){
        setTickets(tickets.sort((a, b) => Object.keys(b).length - Object.keys(a).length ))
    }
    function convertOneTicket(ticket){
        let ticketConverted = {createdAt: ticket["createdAt"], user: ticket["user"], client: ticket["client"], description: ticket["description"] }
        ticket["Productos"]?.map((product, index) => {
            ticketConverted[[String(index)]] =  String(String(product["quantity"]) + "," + String(product["Código"]))
        })
    ...
    return ( <>
        ...
        <div>
            ...
            {JSON.stringify(allTickets?.length)}
        <>
        </>
